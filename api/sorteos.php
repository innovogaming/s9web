...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    $stmt = $pdo->prepare("SELECT promociones.nombre, promociones.premio, promociones.fecha_premio as fechasorteo, tiendas.nombre as tienda FROM tickets INNER JOIN compras ON tickets.id_compra = compras.id_compra INNER JOIN promociones ON tickets.id_promocion = promociones.id_promocion INNER JOIN tiendas ON promociones.id_tienda = tiendas.id_tienda WHERE tickets.id_usuario = ? GROUP BY promociones.id_promocion");
    $stmt->execute([$id]);
